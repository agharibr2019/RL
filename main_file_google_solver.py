...
for counter in range(1, param.num_runs + 1):
    ...
    for x in event_set:
        ...
        # check if event x is 'order ready' event
            ...
            pickup_time = 60 * (abs(driver.X - order.X_rest) + abs(
                driver.Y - order.Y_rest)) / param.V + param.pickup_service_time / 2
            ...
            if driver_id in list(drivers_set.keys()):
                ...
                if (driver.busy == 0 and driver.last_order == order_id):
                    new_X = \
                    stats.truncnorm.rvs(param.a[0], param.b[0], loc=param.mean[0], scale=param.std[0], size=1).tolist()[
                        0]
                    new_Y = \
                    stats.truncnorm.rvs(param.a[1], param.b[1], loc=param.mean[1], scale=param.std[1], size=1).tolist()[
                        0]

                    driving_time = x[2] + 60 * (abs(driver.X - new_X) + abs(driver.Y - new_Y)) / param.V

                    driver.delta_Y = (param.V * param.decision_interval / 60) / (
                                1 + (abs(new_X - driver.X) / abs(new_Y - driver.Y))) * np.sign(
                        new_Y - driver.Y).tolist()
                    driver.delta_X = (abs(new_X - driver.X) / abs(new_Y - driver.Y)) * abs(driver.delta_Y) * np.sign(
                        new_X - driver.X).tolist()
    ...
    orders_results = pd.DataFrame(
        [[value for attr, value in order.__dict__.items()] for order in completed_orders.values()], \
    ...
    drivers_results = pd.DataFrame(
        [[value for attr, value in driver.__dict__.items()] for driver in signedout_drivers.values()], \
